//! IMemoryStore 与 IEmbeddingModel trait——渐进式记忆抽象
//!
//! 对标 MAF 的 3 层记忆架构：
//! - `VolatileMemoryStore`（会话临时）
//! - `SemanticTextMemory`（向量搜索）
//! - Connector 生态（SQLite/Redis/Pinecone 可替换后端）
//!
//! RAF 当前只有 1 种实现：`SkillMemoryContextProvider` + `MemoryAgent` 把 LLM 当文件编辑器。
//! 本模块定义抽象接口，使文件后端成为 fallback，未来可插入 `VectorMemoryStore`。
//!
//! ## 设计原则
//!
//! **原则 3：Trait 即契约，组合即架构。**
//! 将记忆存储与记忆管理解耦。`MemoryAgent` 专注于"何时合并"，`IMemoryStore` 专注于"如何存取"。
//!
//! **原则 4：零成本抽象可测量。**
//! `IEmbeddingModel` trait 允许注入向量模型，只检索相关片段而非全量读取，
//! 将 Token 消耗从 O(n) 降为 O(k)（k 为相关片段数）。
...
/// 文件系统记忆存储——当前默认实现
///
/// 将记忆条目存储为 Markdown 文件，与现有 `SkillMemoryContextProvider` 兼容。
/// 这是 fallback 实现，未来应被 `VectorMemoryStore` 取代以支持语义检索。
