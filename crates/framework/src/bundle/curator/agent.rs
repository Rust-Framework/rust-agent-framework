...
use super::trace::{
    emit_consolidation_error, emit_consolidation_event, parse_consolidation_output,
    ConsolidationObsLevel, ConsolidationRunContext, ConsolidationStatus, ConsolidationResult,
};
...
pub(crate) async fn run_curator(
    ...
    if messages.is_empty() {
        let skipped = ConsolidationResult {
    ...
    let tools: Vec<Arc<dyn ITool>> = registry.list().into_iter().cloned().collect();
    ...
    let result = parse_consolidation_output(&output, &gaps);
