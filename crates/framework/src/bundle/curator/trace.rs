//! Structured observability for knowledge bundle consolidation runs.
...
pub enum ConsolidationObsLevel {
    Prod,
    Dev,
    Trace,
}

impl ConsolidationObsLevel {
    pub fn from_env() -> Self {
        match std::env::var("KNOWLEDGE_BUNDLE_OBS_LEVEL")
            .unwrap_or_default()
            .to_lowercase()
            .as_str()
        {
...
/// Consolidation outcome status for `bundle.consolidation` events.
...
pub fn parse_consolidation_output(raw: &str, index_gaps: &[IndexGap]) -> ConsolidationResult {
...
/// Emit `bundle.consolidation` structured event via tracing.
pub fn emit_consolidation_event(
    ctx: &ConsolidationRunContext,
    result: &ConsolidationResult,
    level: ConsolidationObsLevel,
) {
    ...
    let log_ok_at_info = matches!(level, ConsolidationObsLevel::Dev | ConsolidationObsLevel::Trace);

    match result.status {
        ConsolidationStatus::Error => {
            tracing::error!(
                event = "bundle.consolidation",
                run_id = %ctx.run_id,
    ...
    if let Some(ref sample) = result.raw_output_sample {
        tracing::debug!(
            event = "bundle.consolidation.raw_output",
            run_id = %ctx.run_id,
...
mod tests {
    ...
    #[test]
    fn parse_ok() {
        let r = parse_consolidation_output("OK", &[]);
    ...
    fn parse_updated_with_chatter() {
        ...
        let r = parse_consolidation_output(raw, &[]);
    ...
    fn parse_pure_chatter_defaults_ok() {
        let r = parse_consolidation_output("今天有什么想聊的？😊", &[]);
        assert_eq!(r.status, ConsolidationStatus::Ok);
    ...
    fn event_contract_index_gap_has_paths() {
        ...
        let r = parse_consolidation_output("OK", &gaps);
