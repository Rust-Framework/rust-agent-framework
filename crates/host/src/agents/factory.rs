...
use rust_agent_framework::{
    ...
    super_brain::SuperBrainContextProvider,
};
...
impl<'a> AgentFactory<'a> {
    ...
    /// - 记忆文件持久化到 `memory_dir`
    fn create_super_brain_provider(&self) -> Option<SuperBrainContextProvider> {
        self.config.super_brain_dir.as_ref().map(|dir| {
            debug!(super_brain_dir = %dir, "Creating Super Brain provider");
            SuperBrainContextProvider::new(dir)
        })
    }
    ...
    fn create_coding_agent(&self) -> Result<Arc<dyn IAgent>> {
        ...
        if let Some(super_brain_provider) = self.create_super_brain_provider() {
            builder = builder.add_context_provider(super_brain_provider);
        }
