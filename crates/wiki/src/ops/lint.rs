...
pub fn run_lint(
    ...
    // Note: articulation-point, bridge, and periphery rules require
    // petgraph-live which is not included in this build.
        ...
        for field_name in &link_fields {
            ...
            for val in doc.get_all(f) {
                ...
                if target.starts_with("wiki://") {
                    if check_cross_wiki {
                        if let Some(wiki_name) = target
                            .strip_prefix("wiki://")
                            .and_then(|r| r.split('/').next())
                        {
                            if !mounted_wiki_names.contains(wiki_name) {
                                findings.push(LintFinding {
                                    path: slug_path(&slug, wiki_root),
                                    slug: slug.clone(),
