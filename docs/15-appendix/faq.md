...
`#[tool]` 支持：
- **异步函数**（`async fn`）：自动生成 ITool 实现、参数 Schema 和反序列化器，`kind()` 默认返回 `"function"`
- **结构体**（`struct`）：需要手动实现 `call(&self, arguments: Value) -> Result<ToolResult>`，宏生成委托的 ITool 实现；可通过 `kind = "web"` 指定分类

可使用 `#[tool(description = "...", kind = "file")]` 自定义分类。内置工具已预设分类（`"web"`、`"file"` 等）。
